<h1><img width="24" height="25" src ="https://www.iris-go.com/images/logo-new-lq-45.png"/> ニュース</h1>

> 🚀 **Iris v14 が近づいています。** 次のメジャーバージョンは、このプロジェクト史上もっとも大きなリリースです。Go のジェネリクスの上に作り直されたフレームワーク、コンパイラが検査するアプリケーションビルダー、検証まで引き受けるリクエストヘルパー、ひとつの中央エラーマップ、30 個の同梱ミドルウェア、そしてリポジトリの中に同梱される一冊の本が含まれます。下の [Iris v14 で何が来るのか](#-iris-v14-で何が来るのか)をご覧ください。
>
...
⚡ Iris は、高速でシンプルでありながら、十分な機能を備えた、非常に効率的な Go 用 Web フレームワークです。

✨ あなたの次のウェブサイトや API のために、美しく表現力豊かで使いやすい基盤を提供します。

🌟 [Iris についての他の人々の意見](https://www.iris-go.com/#review)を読み、このオープンソースプロジェクトに **[スターをつけて](https://github.com/kataras/iris/stargazers)** 応援してください
...
ある [Go 開発者](https://twitter.com/dkuye/status/1532087942696554497)が言ったように、**Iris はあらゆる面であなたを支え、長年にわたって揺るがずに立ち続けています** ⭐
...
30 個のミドルウェアが同梱され、その中には新しいものもあります: `httpcost`（リクエストごとの時間・メモリ・CPU。性能分析や課金に使えます）、`bodylimit`、`compress`、`counter`、`referrer`、`geolocation`、`ipaccess`、`servertiming`。ビューエンジンの実装は [iris-contrib/views](https://github.com/iris-contrib/views) に移りました。
...
公開されたリリース日はまだありません。[リリース](https://github.com/kataras/iris/releases)を見守るか、[@iris_framework](https://twitter.com/iris_framework) をフォローすれば、いちばん早く知ることができます。
...
必要なのは [Go プログラミング言語](https://go.dev/dl/)だけです。
...
<details><summary>既存のプロジェクトにインストールする</summary>
...
Iris には広範で詳細な **[ドキュメント](https://www.iris-go.com/docs)** があり、フレームワークを簡単に使い始められます。

より詳しい技術ドキュメントは [godocs](https://pkg.go.dev/github.com/kataras/iris/v12@v12.2.11) をご覧ください。実行できるコードは、いつでもリポジトリの [./_examples](_examples) サブディレクトリにあります。

### Plexon AI で開発する

[Plexon AI](https://plexon.ai) は Hellenic Development によるクロスプラットフォームの AI コーディングアシスタントで、専用の **Iris スキル**を同梱しています。フレームワークの作者自身が書いた v14 開発ガイドで、ルーティングとマクロ、context API、`rest` ヘルパー、エラー処理、30 個すべてのミドルウェア、組み込みコントローラ、認証 SDK、セキュリティ強化、永続化、キャッシュ、i18n、可観測性、パフォーマンス、テスト、デプロイ、そして v12 から v14 への移行を扱う 19 本のリファレンス文書が含まれます。
...
**Iris 電子書籍**（新版）の PDF とオンラインアクセスを今日[リクエスト](https://www.iris-go.com/#ebookDonateForm)して、Iris の開発に参加できます。

## 🙌 コントリビュート

Iris Web Framework へのあなたの貢献をお待ちしています! Iris プロジェクトへの貢献方法について詳しくは [CONTRIBUTING.md](CONTRIBUTING.md) をご覧ください。

[すべてのコントリビューター一覧](https://github.com/kataras/iris/graphs/contributors)

## 🛡 セキュリティ脆弱性

Iris にセキュリティ上の脆弱性を発見した場合は、[iris-go@outlook.com](mailto:iris-go@outlook.com) までメールをお送りください。すべてのセキュリティ脆弱性は速やかに対処されます。

## 📝 ライセンス

このプロジェクトは、Go プロジェクト自体と同じ [BSD 3-clause ライセンス](LICENSE)の下で提供されます。
